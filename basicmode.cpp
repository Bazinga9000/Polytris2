...
BasicMode::BasicMode(std::vector<bool> eorders, StateEngine* game) {
...
void BasicMode::draw(StateEngine* game) {
...
void BasicMode::handle_events(StateEngine* game) {
...
void BasicMode::update(StateEngine* game) {
...
void BasicMode::drawboardpolyomino(int skin, sf::RenderWindow* window, Polyomino polyomino, double pixelscale, bool translucent, int xoffset, int yoffset) {
...
void BasicMode::drawnextpolyomino(int skin, sf::RenderWindow* window, Polyomino polyomino, double pixelscale, int x, int y) {
	for (int i = 0; i < polyomino.order; ++i) {
...
void BasicMode::playsound(std::string name) {
...
//obsolete don't use
std::array<int, 2> BasicMode::gettexture(int colorid) {
...
void BasicMode::imprint(Polyomino poly) {
...
bool BasicMode::canmove(Polyomino polyomino, int dx, int dy) {
...
void BasicMode::movepoly(Polyomino &polyomino, int dx, int dy) {
...
void BasicMode::flatten(Polyomino &poly) {
	int bestrot = 0;
	int minheight = 99999;
	for (int i = 1; i < 5; i++) {
...
bool BasicMode::rotate(Polyomino &polyomino, int numrots) {
...
void BasicMode::harddrop(Polyomino &poly) {
	while (canmove(poly, 0, 1)) {
		poly.row++;
	}
}

void BasicMode::clearlines() {
	//check for spin
	bool spin = !(
	...
	std::vector<std::string> names = {
		"Spin ","Single ","Double ","Triple ","Tesseris ","Pentris ","Hextris ","Heptris ","Octris ","Ennetris ","Decatris ","Hendecatris ","Dodecatris "
	};
	...
	if (linescleared > 0 && !spin) {
		sf::Color c = dtextcolors[linescleared - 1];
		dtext = { colortext(names[linescleared],c,150) };
	}
	else if (spin) {
		sf::Color c = polycolors[currentpoly.color];
		dtext = { colortext(sf::String(namepoly(currentpoly)) + sf::String("-" + names[linescleared]),c,150) };
	}
	...
	playsound("erase" + std::to_string(linescleared));



}

std::wstring BasicMode::namepoly(Polyomino &poly) {
